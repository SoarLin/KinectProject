...
namespace KinectPowerPointControl
{
    ...
    public class KinectSkeleton : Control
    {
        ...
        private readonly Brush RightHandJointBrush = Brushes.SkyBlue;
        ...
        public void RefreshSkeleton(Skeleton skeleton, Dictionary<JointType, JointMapping> mappings, Point center, double scaleFactor)
        {
            ...
            //this.Opacity = 0.25;
            this.InvalidateVisual();  //使項目的呈現失效，並強制新的完整配置傳遞。 在配置週期完成後，會呼叫 OnRender
        }
        ...
        private void DrawBonesAndJoints(DrawingContext drawingContext)
        {
            ...
            if (this.ShowJoints)
            {
                ...
                foreach (JointMapping joint in this.jointMappings.Values)
                {
                    ...
                    if (drawBrush != null)
                    {
                        if (joint.Joint.JointType == JointType.HandRight)
                        {
                            drawBrush = this.RightHandJointBrush;
                            drawingContext.DrawEllipse(drawBrush, null, joint.MappedPoint, JointThickness * 3.0, JointThickness * 3.0);
                        }
                        else
                        {
